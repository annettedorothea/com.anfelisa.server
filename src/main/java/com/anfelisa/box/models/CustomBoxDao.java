...
public class CustomBoxDao {
	...
	/*public List<ICardInfoModel> selectNextCardsByBoxId(Handle handle, String schema, Integer boxId) {
		return handle.createQuery(
				"SELECT b.cardid, c.content, b.count, b.date, b.quality, bos.name, b.timestamp, b.scoredcard FROM "
						+ schema + ".scoredcard b INNER JOIN (SELECT cardid, MAX(count) as maxCount FROM " + schema
						+ ".scoredcard WHERE boxid = :boxId GROUP BY cardid ORDER BY cardid) latestCard ON b.cardid = latestCard.cardid AND b.count = latestCard.maxCount INNER JOIN "
