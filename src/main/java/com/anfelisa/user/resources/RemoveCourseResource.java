...
public class RemoveCourseResource extends Resource {
	...
	@Path("/course")
	@PermitAll
	public Response delete(@Auth AuthUser user, @NotNull @QueryParam("courseId") Integer courseId, @NotNull @QueryParam("uuid") String uuid, @NotNull @QueryParam("schema") String schema) throws JsonProcessingException {
		DatabaseHandle handle = this.createDatabaseHandle();
		RemoveCourseData actionParam = new RemoveCourseData(uuid, schema).withCourseId(courseId).withUsername(user.getUsername());
