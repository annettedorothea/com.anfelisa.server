...
public class CustomLessonDao {
	...
	static final Logger LOG = LoggerFactory.getLogger(CustomLessonDao.class);

	public List<ILessonModel> selectLessons(Handle handle, String courseId) {
	...
	public List<IMyLessonModel> selectMyLessons(Handle handle, String courseId, String username) {
		return handle.createQuery("SELECT l.*, (SELECT count(*) as openTests from public.lesson "
				...
				+ "FROM public.lesson l WHERE l.courseId = :courseId ORDER By sequence").bind("courseId", courseId)
				.bind("username", username).map(new CustomMyLessonMapper()).list();
	}

	public ILessonModel selectByTestId(Handle handle, String testId) {
	...
	public ILessonModel selectByLessonIdAndUsername(Handle handle, String lessonId, String username) {
		return handle.createQuery("SELECT l.* FROM public.course c, public.studentofcourse sc, public.lesson l "
