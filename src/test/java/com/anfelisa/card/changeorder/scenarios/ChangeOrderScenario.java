...
public class ChangeOrderScenario extends AbstractChangeOrderScenario {
	...
	protected void verifications() {
		...
		ICardModel expectedCard = new CardModel("c3", "3given", "3wanted", null, "Annette", 1, "cat1", "boxId");
		assertThat(actualCard, expectedCard);

		actualCard = this.daoProvider.getCardDao().selectByCardId(handle, "c4");
		expectedCard = new CardModel("c4", "4given4", "4wanted4", null, "Annette", 2, "cat1", "boxId");
		assertThat(actualCard, expectedCard);

		actualCard = this.daoProvider.getCardDao().selectByCardId(handle, "c1");
		expectedCard = new CardModel("c1", "given", "wanted", "image", "Annette", 3, "cat1", "boxId");
		assertThat(actualCard, expectedCard);

		actualCard = this.daoProvider.getCardDao().selectByCardId(handle, "c2");
		expectedCard = new CardModel("c2", "given2", "wanted2", "image2", "Annette", 4, "cat1", "boxId");
		assertThat(actualCard, expectedCard);
