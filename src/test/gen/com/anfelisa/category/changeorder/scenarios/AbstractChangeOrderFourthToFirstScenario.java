...
public abstract class AbstractChangeOrderFourthToFirstScenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		com.anfelisa.category.ActionCalls.callCreateCategory("cat3", this.templateStringValue("level 1 #3", 4), this.templateStringValue("boxId", 4), DROPWIZARD.getLocalPort(), authorization("Annette", "password"));

		com.anfelisa.category.ActionCalls.callCreateCategory("cat4", this.templateStringValue("level 1 #4", 5), this.templateStringValue("boxId", 5), DROPWIZARD.getLocalPort(), authorization("Annette", "password"));

	}
	
	private Response when() throws Exception {
		return com.anfelisa.category.ActionCalls.callChangeOrderCategory(randomUUID(), this.templateStringValue("cat4", 0), this.templateStringValue("cat1", 0), DROPWIZARD.getLocalPort(), authorization("Annette", "password"));
	}
	...
	private void then(Response response) throws Exception {
		assertThat(response.getStatus(), 200);
				...
				@Test
				public void changeOrderFourthToFirst() throws Exception {
					given();
