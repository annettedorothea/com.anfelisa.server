...
public abstract class AbstractMoveCardsNoAccessToCardScenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		NotReplayableDataProvider.put("token", this.templateStringValue("ADMIN-TOKEN", null));
		com.anfelisa.user.ActionCalls.callRegisterUser("uuid-admin", this.templateStringValue("admin-password", 5), this.templateStringValue("Admin", 5), this.templateStringValue("annette.pohl@anfelisa.de", 5), this.templateStringValue("de", 5), DROPWIZARD.getLocalPort());

		com.anfelisa.box.ActionCalls.callCreateBox("adminBox", this.templateStringValue("adminBox", 6), new Boolean("false"), null, null, 10, null, DROPWIZARD.getLocalPort(), authorization("Admin", "admin-password"));

		com.anfelisa.category.ActionCalls.callCreateCategory("adminCat", this.templateStringValue("c", 7), this.templateStringValue("adminBox", 7), DROPWIZARD.getLocalPort(), authorization("Admin", "admin-password"));

	}
	
	private Response when() throws Exception {
		return com.anfelisa.card.ActionCalls.callMoveCards(randomUUID(), new ArrayList<>(Arrays.asList(new String[] { "c1" })), 
		this.templateStringValue("adminCat", 0), DROPWIZARD.getLocalPort(), authorization("Admin", "admin-password"));
	}
