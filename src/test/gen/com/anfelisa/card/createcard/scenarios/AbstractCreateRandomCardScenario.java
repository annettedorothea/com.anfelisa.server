...
public abstract class AbstractCreateRandomCardScenario extends BaseScenario {
	...
	private Response when() throws Exception {
		com.anfelisa.card.data.CardCreationTestData createCard0 = new com.anfelisa.card.data.CardCreationTestData(randomUUID());
		createCard0.setCategoryId(this.templateStringValue("cat1", 0));
		createCard0.setGiven(this.templateStringValue("${index}given${random}", 0));
		createCard0.setWanted(this.templateStringValue("${index}wanted${random}", 0));
	...
	private void then(Response response) throws Exception {
		assertThat(response.getStatus(), 200);
				...
				@Test
				public void createRandomCard() throws Exception {
					given();
