...
public abstract class AbstractScoreOtherReinforceCardScenario extends BaseScenario {
	...
	private Response when() throws Exception {
		String uuid = "other-${testId}".replace("${testId}", this.getTestId());
	...
	private void then(Response response) throws Exception {
		...
		if (response.getStatus() != 200) {
			...
			assertFail(message);
				...
				@Test
				public void scoreOtherReinforceCard() throws Exception {
					given();
					...
					if (prerequisite("ScoreOtherReinforceCard")) {
						...
						LOG.info("prerequisite for ScoreOtherReinforceCard not met");
					}
				}
