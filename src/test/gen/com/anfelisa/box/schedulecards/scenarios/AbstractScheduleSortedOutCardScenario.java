...
public abstract class AbstractScheduleSortedOutCardScenario extends BaseScenario {

	static final Logger LOG = LoggerFactory.getLogger(AbstractScheduleSortedOutCardScenario.class);
	...
	private void given() throws Exception {
		...
		if (prerequisite("SortCardsOutWithScoredAndReinforce")) {
			...
			com.anfelisa.box.data.SortCardsOutData data_12 = objectMapper.readValue("{" +
				"\"uuid\" : \"" + uuid + "\"," + 
					"\"cardIds\" : [ \"c1-" + this.getTestId() + "\"," + 
					"\"c4-" + this.getTestId() + "\"]} ",
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: SortCardsOutWithScoredAndReinforce fails due to {} in {} ms", message, (timeAfterRequest-timeBeforeRequest));
				addToMetrics("SortCardsOut", (timeAfterRequest-timeBeforeRequest));
				assertFail(message);
			}
			LOG.info("GIVEN: SortCardsOutWithScoredAndReinforce success in {} ms", (timeAfterRequest-timeBeforeRequest));
			addToMetrics("SortCardsOut", (timeAfterRequest-timeBeforeRequest));
		} else {
	...
	private Response when() throws Exception {
		...
		this.callNotReplayableDataProviderPutSystemTime(uuid, LocalDateTime.parse("20200420 09:30", DateTimeFormatter.ofPattern("yyyyMMdd HH:mm")));
