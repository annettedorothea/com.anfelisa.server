...
public abstract class AbstractGetBoxesTwoDaysLaterScenario extends BaseScenario {
	...
	static final Logger LOG = LoggerFactory.getLogger(AbstractGetBoxesTwoDaysLaterScenario.class);
	...
	private void given() throws Exception {
		...
		if (prerequisite("InitMyBoxesForDayWithScoredAndReinforceCardsTwoDaysLaterPostponesCards")) {
			...
			LOG.info("GIVEN: InitMyBoxesForDayWithScoredAndReinforceCardsTwoDaysLaterPostponesCards uuid " + uuid);
			com.anfelisa.box.data.InitMyBoxesDataData data_13 = objectMapper.readValue("{" +
				"\"uuid\" : \"" + uuid + "\"," + 
					"\"todayAtMidnightInUTC\" : \"2020-04-20T00:00:00.000Z\"} ",
			com.anfelisa.box.data.InitMyBoxesDataData.class);
			response = 
			this.httpPut(
				"/box/init?uuid=" + data_13.getUuid() + "", 
				data_13,
				authorization("Annette-${testId}", "password")
			);
			
			if (response.getStatus() >= 400) {
				String message = "GIVEN InitMyBoxesForDayWithScoredAndReinforceCardsTwoDaysLaterPostponesCards fails\n" + response.readEntity(String.class);
				LOG.info("GIVEN: InitMyBoxesForDayWithScoredAndReinforceCardsTwoDaysLaterPostponesCards fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: InitMyBoxesForDayWithScoredAndReinforceCardsTwoDaysLaterPostponesCards success");
		} else {
			LOG.info("GIVEN: prerequisite for InitMyBoxesForDayWithScoredAndReinforceCardsTwoDaysLaterPostponesCards not met");
		}
		...
		assertThat(actual, expected);
			...
			return actual;
				...
				public void getBoxesTwoDaysLater() throws Exception {
					...
					if (prerequisite("GetBoxesTwoDaysLater")) {
						...
						LOG.info("prerequisite for GetBoxesTwoDaysLater not met");
					}
				}
