...
public abstract class AbstractScoreCardMinimumQFactorScenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		if (prerequisite("ScoreCard310")) {
			uuid = "score310-${testId}".replace("${testId}", this.getTestId());
			LOG.info("GIVEN: ScoreCard310 uuid " + uuid);
			this.callNotReplayableDataProviderPutSystemTime(uuid, DateTime.parse("20220601 16:30", DateTimeFormat.forPattern("yyyyMMdd HH:mm")).withZone(DateTimeZone.UTC), 
						this.getProtocol(), this.getHost(), this.getPort());
			response = 
			com.anfelisa.box.ActionCalls.callScoreCard(objectMapper.readValue("{" +
				"\"uuid\" : \"" + uuid + "\"," + 
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: ScoreCard310 fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreCard310 success");
		} else {
	...
	private Response when() throws Exception {
		String uuid = "q-${testId}".replace("${testId}", this.getTestId());
		this.callNotReplayableDataProviderPutSystemTime(uuid, DateTime.parse("20221001 16:30", DateTimeFormat.forPattern("yyyyMMdd HH:mm")).withZone(DateTimeZone.UTC), 
					this.getProtocol(), this.getHost(), this.getPort());
			...
			assertFail(message);
				...
				public void scoreCardMinimumQFactor() throws Exception {
					...
					if (prerequisite("ScoreCardMinimumQFactor")) {
						...
						LOG.info("prerequisite for ScoreCardMinimumQFactor not met");
					}
				}
