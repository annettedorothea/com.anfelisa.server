...
public abstract class AbstractScoreCardMaxCardsPerDayScenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		if (prerequisite("UpdateBoxSetMaxCardsTo1")) {
			...
			LOG.info("GIVEN: UpdateBoxSetMaxCardsTo1 uuid " + uuid);
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: UpdateBoxSetMaxCardsTo1 fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: UpdateBoxSetMaxCardsTo1 success");
		} else {
		...
		if (prerequisite("ScoreCardSecond0")) {
			uuid = "score0-2-${testId}".replace("${testId}", this.getTestId());
			LOG.info("GIVEN: ScoreCardSecond0 uuid " + uuid);
			this.callNotReplayableDataProviderPutSystemTime(uuid, DateTime.parse("20200418 16:30", DateTimeFormat.forPattern("yyyyMMdd HH:mm")).withZone(DateTimeZone.UTC), 
						this.getProtocol(), this.getHost(), this.getPort());
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: ScoreCardSecond0 fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreCardSecond0 success");
		} else {
	...
	private Response when() throws Exception {
		String uuid = "score0-3-${testId}".replace("${testId}", this.getTestId());
		this.callNotReplayableDataProviderPutSystemTime(uuid, DateTime.parse("20200418 16:30", DateTimeFormat.forPattern("yyyyMMdd HH:mm")).withZone(DateTimeZone.UTC), 
					this.getProtocol(), this.getHost(), this.getPort());
			...
			assertFail(message);
				...
				public void scoreCardMaxCardsPerDay() throws Exception {
					...
					if (prerequisite("ScoreCardMaxCardsPerDay")) {
						...
						LOG.info("prerequisite for ScoreCardMaxCardsPerDay not met");
					}
				}
