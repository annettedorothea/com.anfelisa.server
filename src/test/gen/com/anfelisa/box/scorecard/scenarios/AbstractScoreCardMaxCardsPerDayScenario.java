...
public abstract class AbstractScoreCardMaxCardsPerDayScenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		if (prerequisite("UpdateBoxSetMaxCardsTo1")) {
			...
			com.anfelisa.box.data.BoxUpdateData data_9 = objectMapper.readValue("{" +
				"\"uuid\" : \"" + uuid + "\"," + 
			...
			response = 
			this.httpPut(
				"/box/update?uuid=" + data_9.getUuid() + "", 
				data_9,
				authorization("Annette-${testId}", "password")
			);
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: UpdateBoxSetMaxCardsTo1 fails due to {} in {} ms", message, (timeAfterRequest-timeBeforeRequest));
				addToMetrics("UpdateBox", (timeAfterRequest-timeBeforeRequest));
				assertFail(message);
			}
			LOG.info("GIVEN: UpdateBoxSetMaxCardsTo1 success in {} ms", (timeAfterRequest-timeBeforeRequest));
			addToMetrics("UpdateBox", (timeAfterRequest-timeBeforeRequest));
		} else {
		...
		if (prerequisite("ScheduleCards")) {
			...
			com.anfelisa.box.data.ScheduledCardsData data_10 = objectMapper.readValue("{" +
				"\"uuid\" : \"" + uuid + "\"," + 
		...
		if (prerequisite("ScoreCardSecond0")) {
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: ScoreCardSecond0 fails due to {} in {} ms", message, (timeAfterRequest-timeBeforeRequest));
				addToMetrics("ScoreCard", (timeAfterRequest-timeBeforeRequest));
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreCardSecond0 success in {} ms", (timeAfterRequest-timeBeforeRequest));
			addToMetrics("ScoreCard", (timeAfterRequest-timeBeforeRequest));
		} else {
