...
public abstract class AbstractLoadNextCardReinforceScored1Scenario extends BaseScenario {

	static final Logger LOG = LoggerFactory.getLogger(AbstractLoadNextCardReinforceScored1Scenario.class);
	...
	private void given() throws Exception {
		...
		if (prerequisite("ScoreReinforceCard1")) {
			uuid = "sr1-${testId}".replace("${testId}", this.getTestId());
			LOG.info("GIVEN: ScoreReinforceCard1 uuid " + uuid);
			...
			if (response.getStatus() >= 400) {
				String message = "GIVEN ScoreReinforceCard1 fails\n" + response.readEntity(String.class);
				LOG.info("GIVEN: ScoreReinforceCard1 fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreReinforceCard1 success");
		} else {
			LOG.info("GIVEN: prerequisite for ScoreReinforceCard1 not met");
		}
			...
			return actual;
				...
				public void loadNextCardReinforceScored1() throws Exception {
					...
					if (prerequisite("LoadNextCardReinforceScored1")) {
						...
						LOG.info("prerequisite for LoadNextCardReinforceScored1 not met");
					}
				}
				...
				protected String scenarioName() {
					return "LoadNextCardReinforceScored1";
				}
