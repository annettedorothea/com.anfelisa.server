...
public abstract class AbstractLoadNextCardReinforceScored2Scenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		if (prerequisite("ScoreReinforceCard2")) {
			uuid = "sr2-${testId}".replace("${testId}", this.getTestId());
			LOG.info("GIVEN: ScoreReinforceCard2 uuid " + uuid);
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: ScoreReinforceCard2 fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreReinforceCard2 success");
		} else {
	...
	private com.anfelisa.box.data.LoadNextCardResponse then(Response response) throws Exception {
		...
		assertThat(actual, expected);
			...
			return actual;
				...
				@Test
				public void loadNextCardReinforceScored2() throws Exception {
					given();
					...
					if (prerequisite("LoadNextCardReinforceScored2")) {
						...
						LOG.info("prerequisite for LoadNextCardReinforceScored2 not met");
					}
				}
