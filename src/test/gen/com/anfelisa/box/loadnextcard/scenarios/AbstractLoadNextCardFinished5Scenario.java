...
public abstract class AbstractLoadNextCardFinished5Scenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		if (prerequisite("ScoreReinforceCard0")) {
			...
			LOG.info("GIVEN: ScoreReinforceCard0 uuid " + uuid);
		...
		if (prerequisite("ScoreReinforceCard5")) {
			...
			LOG.info("GIVEN: ScoreReinforceCard5 uuid " + uuid);
			this.callNotReplayableDataProviderPutSystemTime(uuid, DateTime.parse("20200418 16:40", DateTimeFormat.forPattern("yyyyMMdd HH:mm")).withZone(DateTimeZone.UTC), 
						this.getProtocol(), this.getHost(), this.getPort());
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: ScoreReinforceCard5 fails due to " + message);
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreReinforceCard5 success");
		} else {
	...
	private com.anfelisa.box.data.LoadNextCardResponse then(Response response) throws Exception {
		...
		assertThat(actual, expected);
			...
			return actual;
				...
				@Test
				public void loadNextCardFinished5() throws Exception {
					given();
					...
					if (prerequisite("LoadNextCardFinished5")) {
						...
						LOG.info("prerequisite for LoadNextCardFinished5 not met");
					}
				}
