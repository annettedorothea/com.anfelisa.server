...
public abstract class AbstractLoadNextCardFinished5Scenario extends BaseScenario {

	static final Logger LOG = LoggerFactory.getLogger(AbstractLoadNextCardFinished5Scenario.class);
	...
	private void given() throws Exception {
		...
		if (prerequisite("ScoreReinforceCard0")) {
			...
			if (response.getStatus() >= 400) {
				...
				LOG.info("GIVEN: ScoreReinforceCard0 fails due to {} in {} ms", message, (timeAfterRequest-timeBeforeRequest));
				addToMetrics("ScoreReinforceCard", (timeAfterRequest-timeBeforeRequest));
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreReinforceCard0 success in {} ms", (timeAfterRequest-timeBeforeRequest));
			addToMetrics("ScoreReinforceCard", (timeAfterRequest-timeBeforeRequest));
		} else {
		...
		if (prerequisite("ScoreReinforceCard5")) {
			...
			timeAfterRequest = System.currentTimeMillis();
			if (response.getStatus() >= 400) {
				String message = "GIVEN ScoreReinforceCard5 fails\n" + response.readEntity(String.class);
				LOG.info("GIVEN: ScoreReinforceCard5 fails due to {} in {} ms", message, (timeAfterRequest-timeBeforeRequest));
				addToMetrics("ScoreReinforceCard", (timeAfterRequest-timeBeforeRequest));
				assertFail(message);
			}
			LOG.info("GIVEN: ScoreReinforceCard5 success in {} ms", (timeAfterRequest-timeBeforeRequest));
			addToMetrics("ScoreReinforceCard", (timeAfterRequest-timeBeforeRequest));
		} else {
			LOG.info("GIVEN: prerequisite for ScoreReinforceCard5 not met");
		}
	...
	public void runTest() throws Exception {
		...
		if (prerequisite("LoadNextCardFinished5")) {
