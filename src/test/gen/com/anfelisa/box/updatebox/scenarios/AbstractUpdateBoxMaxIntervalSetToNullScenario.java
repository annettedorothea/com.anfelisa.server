...
public abstract class AbstractUpdateBoxMaxIntervalSetToNullScenario extends BaseScenario {
	...
	private void given() throws Exception {
		...
		if (prerequisite("UpdateBoxMaxInterval")) {
			...
			com.anfelisa.box.data.BoxUpdateData data_3 = objectMapper.readValue("{" +
				"\"uuid\" : \"" + uuid + "\"," + 
			...
			response = 
			this.httpPut(
				"/box/update?uuid=" + data_3.getUuid() + "", 
				data_3,
				authorization("Annette-${testId}", "password")
			);
			...
			assertFail(message);
				...
				public void updateBoxMaxIntervalSetToNull() throws Exception {
					...
					if (prerequisite("UpdateBoxMaxIntervalSetToNull")) {
						...
						LOG.info("prerequisite for UpdateBoxMaxIntervalSetToNull not met");
					}
				}
