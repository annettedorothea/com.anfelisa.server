...
public class ResultDao {
	...
	public void create(Handle handle) {
		handle.execute("CREATE TABLE IF NOT EXISTS public.result (resultid integer NOT NULL  , username character varying NOT NULL  , testid integer NOT NULL  , date timestamp with time zone NOT NULL  , json character varying NOT NULL  , points integer NOT NULL  , maxpoints integer NOT NULL  , CONSTRAINT result_pkey PRIMARY KEY (resultid), CONSTRAINT result_username_fkey FOREIGN KEY (username) REFERENCES public.user ( username ) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE, CONSTRAINT result_testid_fkey FOREIGN KEY (testid) REFERENCES public.test ( testid ) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE, CONSTRAINT result_resultId_unique UNIQUE (resultId))");
	}
	
	public Integer insert(Handle handle, IResultModel resultModel) {
		Query<Map<String, Object>> statement = handle.createQuery("INSERT INTO public.result (resultid, username, testid, date, json, points, maxpoints) VALUES ( (SELECT COALESCE(MAX(resultid),0) + 1 FROM public.result), :username, :testid, :date, :json, :points, :maxpoints) RETURNING resultid");
