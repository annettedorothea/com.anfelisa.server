...
public abstract class AbstractDeleteBoxCommand extends Command<IDeleteBoxData> {
	...
	public void publishEvents(IDeleteBoxData data, PersistenceHandle handle, PersistenceHandle timelineHandle) {
		if (data.hasOutcome("deleteBox")){
			new Event<IDeleteBoxData>("com.anfelisa.box.events.DeleteBoxDeleteBoxEvent", daoProvider, viewProvider, appConfiguration).publish(data.deepCopy(), handle, timelineHandle);
		}
		if (data.hasOutcome("deleteCategory")){
			new Event<IDeleteBoxData>("com.anfelisa.box.events.DeleteBoxDeleteCategoryEvent", daoProvider, viewProvider, appConfiguration).publish(data.deepCopy(), handle, timelineHandle);
		}
	}
