...
public abstract class AbstractRegisterUserCommand extends Command<IUserRegistrationData> {
	...
	public void publishEvents(IUserRegistrationData data, PersistenceHandle handle, PersistenceHandle timelineHandle) {
		if (data.hasOutcome("ok")){
			new Event<IUserRegistrationData>("com.anfelisa.user.events.RegisterUserOkEvent", daoProvider, viewProvider, appConfiguration).publish(data.deepCopy(), handle, timelineHandle);
		}
	}
	...
	public void publishAfterCommitEvents(IUserRegistrationData data, PersistenceHandle handle, PersistenceHandle timelineHandle) {
		if (data.hasOutcome("ok")){
			new Event<IUserRegistrationData>("com.anfelisa.user.events.RegisterUserOkEvent", daoProvider, viewProvider, appConfiguration).publishAfterCommit(data.deepCopy(), handle, timelineHandle);
		}
	}
